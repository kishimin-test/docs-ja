...
担当：実際の依存範囲に基づいてサイズを個別分類したE2Eテスト

PlaywrightやE2Eというテスト種別だけを理由にLargeへ分類しない。localhostで起動したフロントエンドおよびAPIスタブへ接続するテストはMedium、localhost以外のデプロイ済みAPIや実サービスへ接続するテストはLargeとする。VRTやダウンロード検証でファイルシステムを使用するテストは、ほかの依存がなくても少なくともMediumとする。

- 実ブラウザでの画像生成フロー
- localhostのAPIスタブまたはデプロイ済みAPIを使用した `POST /images`
- PNGダウンロード
- レスポンシブ表示
- 404とエラー画面
- キーボード操作
...
- `test/mojica-ui-e2e`の各テストを実際の依存範囲でSmall、Medium、Largeのいずれかへ分類し、サイズをファイル名へ明示する。親ブランチへマージする前に、その時点で実行可能な全サイズの結果を確認する。
